...
export default function DashboardPage() {
  ...
  const { priorityData, statusData, severityData, productData, slaBarData } = chartData;
            ...
            <div className="space-y-5">
              <div className="card p-3">
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    onClick={() => setShowAnaFilter(!showAnaFilter)}
                  ...
                  <div className="mt-3 flex flex-wrap gap-3 pt-3 border-t border-gray-100">
                    <div>
                      ...
                      <input
                        type="date"
                        value={anaDateFrom}
                        onChange={(e) => setAnaDateFrom(e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={anaDateTo}
                        onChange={(e) => setAnaDateTo(e.target.value)}
                        className="input-field text-sm"
                      />
                    </div>
                  </div>
              ...
              {anaLoading ? (
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <KpiCard
                    label="SLA Breached"
                    value={summary.breached_tickets}
                    icon={ShieldAlert}
                    color="text-red-600"
                    bg="bg-red-50"
                    sub={
                      sla
                        ? `${formatPercent(100 - sla.resolution_compliance_pct)} breach rate`
                        : undefined
                    }
                  />
                  <KpiCard
                    ...
                    bg="bg-orange-50"
                    onClick={() => navigate('/tickets/escalated')}
                  />
                  <KpiCard
                    label="On Hold"
                    value={summary.on_hold_tickets}
                    icon={Clock}
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-4">
                        <div className="bg-green-50 rounded-xl p-3 text-center">
                          <p className="text-2xl font-bold text-green-600">
                            {formatPercent(sla?.response_compliance_pct ?? 0)}
                          </p>
                          <p className="text-xs text-gray-500 mt-0.5">Response SLA met</p>
                          <p className="text-xs text-red-400 mt-0.5">
                            {sla?.response_sla_breached ?? 0} breached
                          </p>
                        </div>
                        <div className="bg-blue-50 rounded-xl p-3 text-center">
                          <p className="text-2xl font-bold text-blue-600">
                            {formatPercent(sla?.resolution_compliance_pct ?? 0)}
                          </p>
                          <p className="text-xs text-gray-500 mt-0.5">Resolution SLA met</p>
                          <p className="text-xs text-red-400 mt-0.5">
                            {sla?.resolution_sla_breached ?? 0} breached
                          </p>
                        </div>
                      </div>
                      {slaBarData.length > 0 && (
                        ...
                        <table className="w-full text-sm">
                          ...
                          <tbody className="divide-y divide-gray-50">
                            {topAgents.map((a) => {
                              const rate =
                                a.total_assigned > 0
                                  ? (a.total_resolved / a.total_assigned) * 100
                                  : 0;
                              return (
                                <tr
                                  key={a.agent_user_id}
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3 text-gray-600">
                                    {a.avg_resolution_minutes
                                      ? formatMinutes(Math.round(a.avg_resolution_minutes))
                                      : '—'}
                                  </td>
                                  <td className="px-4 py-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      ...
                      },
                      {
                        ...
                        desc: 'Configure teams & assignments',
                        path: '/teams',
                        icon: UserPlus,
                        color: 'indigo',
                      },
            ...
            <div className="space-y-4">
              <div className="card p-4 space-y-3">
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search by ticket ID, subject, or keywords…"
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    {Object.entries(filters)
                      ...
                      .map(([key, val]) => (
                        <span
                          key={key}
                          className="inline-flex items-center gap-1 px-2.5 py-1 bg-violet-50 text-violet-700 text-xs font-medium rounded-full border border-violet-200"
                        >
              ...
              <SectionCard
                title={
                  srLoading ? 'Loading…' : `${srTotal} ticket${srTotal !== 1 ? 's' : ''} found`
                }
                noPad
              >
                <div className="overflow-x-auto">
                            ...
                            <td className="px-4 py-3 text-xs">
                              {ticket.assignee_id ? (
                                <span className="font-medium text-gray-700">
                                  {srNameCache[ticket.assignee_id] ??
              ...
              {analytics && !anaLoading && (
                <>
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <KpiCard
                      label="Response SLA %"
                      value={sla ? formatPercent(sla.response_compliance_pct) : '—'}
                      icon={CheckCircle}
                      color="text-green-600"
                      bg="bg-green-50"
                    />
                    <KpiCard
                      ...
                      value={sla ? formatPercent(sla.resolution_compliance_pct) : '—'}
                      icon={CheckCircle}
                        ...
                        {[
                          {
                            ...
                            value: sla?.response_sla_met ?? 0,
                            pct: sla?.response_compliance_pct ?? 0,
                            color: 'bg-green-400',
                          },
                          {
                            label: 'Response SLA breached',
                            value: sla?.response_sla_breached ?? 0,
                            pct: sla
                              ? (sla.response_sla_breached / (sla.total_tickets || 1)) * 100
                              : 0,
                            color: 'bg-red-400',
                          },
                          {
                            label: 'Resolution SLA met',
                            value: sla?.resolution_sla_met ?? 0,
                            pct: sla?.resolution_compliance_pct ?? 0,
                            color: 'bg-blue-400',
                          },
                          {
                            label: 'Resolution breached',
                            value: sla?.resolution_sla_breached ?? 0,
                            pct: sla
                              ? (sla.resolution_sla_breached / (sla.total_tickets || 1)) * 100
                              : 0,
                            color: 'bg-amber-400',
                          },
                        ].map((row) => (
                            ...
                            <div className="flex-1 h-5 bg-gray-100 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width:
                                    priorityData.length &&
                                    Math.max(...priorityData.map((x) => x.tickets)) > 0
                                      ? `${(p.tickets / Math.max(...priorityData.map((x) => x.tickets))) * 100}%`
                                      : '0%',
