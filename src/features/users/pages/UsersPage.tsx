...
export default function UsersPage() {
  ...
  const modalFooter =
    modalMode === 'view' || modalMode === 'skills' ? (
      ...
      <>
        ...
        <button
          onClick={handleCreateUserAndClose}
          disabled={saving}
      ...
      <div className="card p-1 inline-flex rounded-xl gap-1 flex-wrap">
        {(
          ...
          >
            ...
            {label}
            <span
              className={`ml-0.5 px-1.5 py-0.5 rounded-full text-xs font-semibold ${activeTab === tab ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-500'}`}
            >
              {count}
            </span>
          </button>
                ...
                {filtered.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="table-row">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${ts.bg} ${ts.text} ${ts.border}`}
                        >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`badge ${u.is_verified ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}
                        >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <span
                  className={`badge ${selectedUser.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                >
              ...
              <div>
                ...
                <span
                  className={`badge ${selectedUser.is_verified ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}
                >
            ...
            {selectedUser.lead_id && (
              <div>
                ...
                <p className="text-sm text-gray-900">{getUserName(selectedUser.lead_id)}</p>
              </div>
            )}
                ...
                {skillsLoading ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {agentSkills.map((skill) => {
                      const prof = PROFICIENCY_LEVELS.find(
                      ...
                      return (
                        ...
                        >
                          ...
                          {skill.area_name}
                          <span
                            className={`px-1.5 py-0.5 rounded-full text-[10px] font-semibold ${prof?.color || 'bg-gray-100 text-gray-700'}`}
                          >
              ...
              <div className="border border-gray-200 rounded-lg p-4 space-y-3 bg-gray-50/50">
                ...
                {createSkills.length > 0 && (
                  <div className="space-y-2">
                    {createSkills.map((skill) => {
                      const areaName =
                        areas.find((a) => a.area_id === skill.area_id)?.name ??
                        `Area ${skill.area_id}`;
                      const prof = PROFICIENCY_LEVELS.find(
                        (p) => p.value === skill.proficiency_level
                      );
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${prof?.color || 'bg-gray-100 text-gray-700'}`}
                            >
                              {prof?.label || skill.proficiency_level}
                            </span>
                          </div>
                          <button
                            type="button"
                            onClick={() => handleRemoveCreateSkill(skill.area_id)}
                ...
                {availableCreateAreas.length > 0 && (
                  <div className="flex items-end gap-2">
                    <div className="flex-1">
                      ...
                      <select
                        value={createNewSkill.area_id}
                        onChange={(e) =>
                          setCreateNewSkill((p) => ({ ...p, area_id: parseInt(e.target.value) }))
                        }
                        className="input-field text-sm"
                      >
                        <option value={0}>Select an area...</option>
                        {availableCreateAreas.map((a) => (
                    ...
                    <div className="flex-1">
                      ...
                      <select
                        value={createNewSkill.proficiency_level}
                        onChange={(e) =>
                          setCreateNewSkill((p) => ({ ...p, proficiency_level: e.target.value }))
                        }
                        className="input-field text-sm"
                      >
                    ...
                    <button
                      ...
                      disabled={!createNewSkill.area_id}
                ...
                {availableCreateAreas.length === 0 && createSkills.length > 0 && (
                  <p className="text-xs text-gray-500 italic">
                    All available areas have been assigned.
                  </p>
                )}
              </div>
            )}
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            handleUpdateSkillProficiency(skill.area_id, e.target.value)
                          }
                          disabled={saving}
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewSkill((p) => ({ ...p, area_id: parseInt(e.target.value) }))
                      }
                      className="input-field"
                    >
                      <option value={0}>Select an area...</option>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewSkill((p) => ({ ...p, proficiency_level: e.target.value }))
                      }
                      className="input-field"
                    >
                      {PROFICIENCY_LEVELS.map((p) => (
