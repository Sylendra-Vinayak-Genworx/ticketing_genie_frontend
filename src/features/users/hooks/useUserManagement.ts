...
export function useUserManagement() {
  ...
  const [userForm, setUserForm]     = useState<Partial<UserCreateRequest & UserUpdateRequest>>({
    email: '', full_name: '', role: 'support_agent', is_active: true,
  })
  ...
  const activeList = useMemo(() => {
    switch (activeTab) {
      ...
      case 'team_lead':     return leadUsers
      case 'support_agent': return agentUsers
      case 'customers':     return customerUsers
    }
  ...
  const availableAreas       = useMemo(() => areas.filter(a => !agentSkills.some(s => s.area_id === a.area_id)), [areas, agentSkills])
  const availableCreateAreas = useMemo(() => areas.filter(a => !createSkills.some(s => s.area_id === a.area_id)), [areas, createSkills])
  ...
  function closeModal() {
    ...
    setAgentSkills([]); setNewSkill(INITIAL_SKILL)
    setCreateSkills([]); setCreateNewSkill(INITIAL_SKILL)
  }
  ...
  async function handleAddSkill() {
    ...
    try {
      await authService.updateUserSkills(selectedUser.id, {
        skills: [...agentSkills.map(s => ({ area_id: s.area_id, proficiency_level: s.proficiency_level })),
                 { area_id: newSkill.area_id, proficiency_level: newSkill.proficiency_level }],
      })
      toast.success('Skill added'); loadAgentSkills(selectedUser.id); setNewSkill(INITIAL_SKILL)
  ...
  async function handleRemoveSkill(areaId: number) {
    ...
    try {
      await authService.updateUserSkills(selectedUser.id, {
        skills: agentSkills.filter(s => s.area_id !== areaId).map(s => ({ area_id: s.area_id, proficiency_level: s.proficiency_level })),
      })
      toast.success('Skill removed'); loadAgentSkills(selectedUser.id)
  ...
  async function handleUpdateSkillProficiency(areaId: number, newLevel: string) {
    ...
    try {
      await authService.updateUserSkills(selectedUser.id, {
        skills: agentSkills.map(s => s.area_id === areaId
          ? { area_id: s.area_id, proficiency_level: newLevel }
          : { area_id: s.area_id, proficiency_level: s.proficiency_level }),
      })
      toast.success('Skill updated'); loadAgentSkills(selectedUser.id)
  ...
  function handleAddCreateSkill() {
    if (!createNewSkill.area_id) { toast.error('Please select an area'); return }
  ...
  async function handleCreateUser() {
    ...
    setSaving(true)
    try {
      const r = await authService.createUser({ email: userForm.email, full_name: userForm.full_name || '', role: userForm.role as UserRole })
  ...
  async function handleCreateUserAndClose() {
    ...
    if (r) {
      toast.success(
        `User created successfully. Temporary password: ${r.temporary_password}`,
        { duration: 10000 }
      )
      closeModal(); loadData()
    }
  ...
  async function handleUpdateUser() {
    ...
    setSaving(true)
    try {
      await authService.updateUser(selectedUser.id, { full_name: userForm.full_name, is_active: userForm.is_active })
      toast.success('User updated successfully'); closeModal(); loadData()
  ...
  return {
    // data
    loading, areas, tiers,
    adminUsers, leadUsers, agentUsers, customerUsers, filtered,
