...
export const authService = {
  ...
  async updateUserSkills(
    userId: string,
    data: AgentSkillUpdateRequest
  ): Promise<AgentSkillListResponse> {
    const res = await ticketingApi.put<AgentSkillListResponse>(
      `/admin/users/${userId}/skills`,
      data
    );
    return res.data;
  },
  ...
  async resetPassword(token: string, new_password: string): Promise<{ message: string }> {
    const res = await authApi.post<{ message: string }>('/auth/reset-password', {
      token,
      new_password,
    });
    return res.data;
