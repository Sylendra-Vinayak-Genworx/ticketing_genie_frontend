...
export default function SLAConfigPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ type: 'sla' | 'rule'; id: number } | null>(
    null
  );
  ...
  function handleAddRuleSubmit() {
    ...
    setSlas((prev) =>
      prev.map((s) => (s.sla_id === addRuleModal ? { ...s, rules: [...s.rules, newRule] } : s))
    );
  ...
  return (
    <div className="space-y-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Tier ID: {sla.customer_tier_id} · {sla.rules.length} rules
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span
                className={`badge ${sla.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}
              >
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setDeleteTarget({ type: 'sla', id: sla.sla_id });
                }}
          ...
          {expanded.has(sla.sla_id) && (
            <div className="border-t border-gray-100">
              <div className="flex justify-end p-3 border-b border-gray-100 bg-gray-50/50">
                <button
                  onClick={() => setAddRuleModal(sla.sla_id)}
                  className="btn-secondary text-sm flex items-center gap-1.5 py-1.5 px-3 bg-white"
                >
                    ...
                    {sla.rules.map((rule) => (
                      <tr key={rule.rule_id} className="table-row">
                        ...
                        <td className="px-4 py-3 text-right font-medium text-gray-800">
                          {formatMinutes(rule.response_time_minutes)}
                        </td>
                        <td className="px-4 py-3 text-right font-medium text-gray-800">
                          {formatMinutes(rule.resolution_time_minutes)}
                        </td>
                        <td className="px-4 py-3 text-right font-medium text-gray-800">
                          {formatMinutes(rule.escalation_after_minutes)}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1 justify-end">
                            ...
                            <button
                              onClick={() => setDeleteTarget({ type: 'rule', id: rule.rule_id })}
                              className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg"
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                            </button>
      ...
      <Modal
        ...
        footer={
          <>
            <button onClick={() => setEditRule(null)} className="btn-secondary">
              Cancel
            </button>
          ...
          <div className="space-y-4">
            ...
            {(
              [
                ...
                ['resolution_time_minutes', 'Resolution Time (minutes)'],
                ['escalation_after_minutes', 'Escalation After (minutes)'],
              ] as const
            ).map(([key, label]) => (
              <div key={key}>
                ...
                <input
                  ...
                  value={editForm[key]}
                  onChange={(e) =>
                    setEditForm((prev) => ({ ...prev, [key]: Number(e.target.value) }))
                  }
      ...
      <Modal
        ...
        footer={
          <>
            <button onClick={() => setAddSlaModal(false)} className="btn-secondary">
              Cancel
            </button>
            <button onClick={handleAddSlaSubmit} className="btn-primary">
              Add Tier
            </button>
          </>
        }
      >
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 pt-2">
            ...
            />
            <label htmlFor="sla-active" className="text-sm font-medium text-gray-700">
              Active
            </label>
          </div>
        </div>
      ...
      <Modal
        ...
        footer={
          <>
            <button onClick={() => setAddRuleModal(null)} className="btn-secondary">
              Cancel
            </button>
            <button onClick={handleAddRuleSubmit} className="btn-primary">
              Add Rule
            </button>
          </>
        }
          ...
          {(
            ...
            <div key={key}>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewRuleForm((prev) => ({ ...prev, [key]: Number(e.target.value) }))
                }
                className="input-field"
              />
            </div>
