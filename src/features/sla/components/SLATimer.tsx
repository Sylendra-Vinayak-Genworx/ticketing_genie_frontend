...
export function SLATimer({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (days > 365) { setTimeLeft('Not Started'); return }

      if (diff <= 0) {
        setTimeLeft(days > 0 ? `${days}d ${hours}h ${minutes}m overdue` : `${hours}h ${minutes}m overdue`)
      } else {
        setTimeLeft(days > 0 ? `${days}d ${hours}h ${minutes}m remaining` : `${hours}h ${minutes}m remaining`)
      }
    }
