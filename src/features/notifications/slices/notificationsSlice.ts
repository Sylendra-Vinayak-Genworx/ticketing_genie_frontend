...
const notificationsSlice = createSlice({
  ...
  reducers: {
    pushNotification(state, action: PayloadAction<SSENotification>) {
      const id = action.payload.notification_id?.toString() || `${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
export const { pushNotification, markAllRead, markRead, clearAll, setConnected } =
