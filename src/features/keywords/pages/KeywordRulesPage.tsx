...
const EMPTY_FORM = {
  keyword: '',
  match_field: 'BOTH' as MatchField,
  target_severity: 'HIGH' as Severity,
  is_active: true,
};
              ...
              {isLoading
                ...
                : rules.map((rule) => (
                    <tr key={rule.keyword_rule_id} className="table-row">
                      ...
                      <td className="px-4 py-3">
                        <SeverityBadge severity={rule.target_severity} />
                      </td>
                      <td className="px-4 py-3">
                        <button
                          onClick={() => toggleActive(rule)}
                          className={`badge cursor-pointer transition-colors ${rule.is_active ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                        >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 justify-end">
                          ...
                          <button
                            onClick={() => setDeleteTarget(rule.keyword_rule_id)}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, match_field: e.target.value as MatchField }))
                }
                className="input-field"
              >
                {MATCH_FIELDS.map((mf) => (
            ...
            <div>
              ...
              <select
                value={form.target_severity}
                onChange={(e) =>
                  setForm((f) => ({ ...f, target_severity: e.target.value as Severity }))
                }
                className="input-field"
              >
                {SEVERITIES.map((s) => (
