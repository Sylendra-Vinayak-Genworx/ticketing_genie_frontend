...
export function useTicketDetail() {
  ...
  const resolveNames = useCallback(
    async (ticket: {
      ...
      if (ticket.assignee_id) {
        userService
          .getUserById(ticket.assignee_id)
          .then((u) => setMeta((m) => ({ ...m, assigneeName: u.full_name || u.email })))
          .catch(() =>
            setMeta((m) => ({ ...m, assigneeName: `Guest (${ticket.assignee_id!.slice(0, 8)})` }))
          );
      } else {
      ...
      userService
        .getUserById(ticket.customer_id)
        .then((u) => setMeta((m) => ({ ...m, customerName: u.full_name || u.email })))
        .catch(() =>
          setMeta((m) => ({ ...m, customerName: `Guest (${ticket.customer_id.slice(0, 8)})` }))
        );
    },
    []
  );
  ...
  const loadAgents = useCallback(
    async (role: string, currentUserId: string, assigneeId?: string | null): Promise<void> => {
      setMeta((m) => ({ ...m, agentsLoading: true, agents: [] }));
      try {
        const leadId = role === 'team_lead' ? currentUserId : (assigneeId ?? null);

        if (!leadId) {
          const all = await userService.getAllUsers();
          setMeta((m) => ({
            ...m,
            agents: all.filter((u) => u.role === 'support_agent'),
            agentsLoading: false,
          }));
          return;
        }
