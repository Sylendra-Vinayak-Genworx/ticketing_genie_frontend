...
export function useTicketDetailPage(ticketId: number | undefined) {
  ...
  const { assigneeName, customerName, areaName, agents, agentsLoading, resolveNames, loadAgents } =
    useTicketDetail();
    ...
    const actorIds = [
      ...new Set([
        ...currentTicket.events
          .map((e) => e.triggered_by_user_id)
          .filter((id): id is string => typeof id === 'string' && !!id),
        ...currentTicket.events
          .filter((e) => e.event_type === 'ASSIGNED')
          .flatMap((e) => [e.new_value, e.old_value])
          .filter((v): v is string => typeof v === 'string' && !!v && UUID_RE.test(v)),
        ...currentTicket.comments
          .map((c) => c.author_id)
          .filter((id): id is string => typeof id === 'string' && !!id && UUID_RE.test(id)),
      ]),
    ];
      ...
      Promise.all(
        actorIds.map((uid) =>
          authService
            .getUserById(uid)
            .then((u) => {
              nameMap[uid] = u.full_name || u.email;
            })
            .catch(() => {
  ...
  async function handleAddComment(e: React.FormEvent) {
    ...
    const result = (await addComment(currentTicket.ticket_id, data)) as { payload?: { comment_id?: number } | string };
    if (typeof result.payload === 'object' && result.payload && 'comment_id' in result.payload) {
      ...
      if (selfEscalate) {
        try {
          ...
          toast.error(error?.response?.data?.detail || 'Comment sent but escalation failed');
        }
      } else {
      ...
      toast.error((typeof result.payload === 'string' ? result.payload : '') || 'Failed to add comment');
    }
  }
  ...
  async function handleStatusChange() {
    ...
    })) as { payload?: { ticket_id?: number } | string; type?: string };
    const isFulfilled =
      (typeof result.payload === 'object' && result.payload && 'ticket_id' in result.payload) ||
    ...
    if (isFulfilled) {
      ...
      toast.error((typeof result.payload === 'string' ? result.payload : '') || 'Failed to update status');
    }
  }

  async function handleAssign() {
    if (!currentTicket || !assigneeId) return;
    const result = (await assign(currentTicket.ticket_id, { assignee_id: assigneeId })) as {
    ...
    if (isFulfilled) {
      ...
      toast.error((typeof result.payload === 'string' ? result.payload : '') || 'Failed to assign ticket');
    }
  }
  ...
  const openStatusModal = useCallback(() => {
    if (!currentTicket) return;
    const options = (ALLOWED_TRANSITIONS[currentTicket.status] ?? []).filter(
      (s) => s !== 'CLOSED' && !(currentTicket.status === 'CLOSED')
    );
    if (options.length > 0) {
      setNewStatus(options[0]);
      setStatusModalOpen(true);
    }
  }, [currentTicket, setNewStatus, setStatusModalOpen]);

  const allowedTransitions = currentTicket ? (ALLOWED_TRANSITIONS[currentTicket.status] ?? []) : [];
  const statusOptions = allowedTransitions.filter(
    (s) => s !== 'CLOSED' && currentTicket?.status !== 'CLOSED'
  );
