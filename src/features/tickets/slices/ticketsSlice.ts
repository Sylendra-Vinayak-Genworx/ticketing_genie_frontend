...
export const fetchMyTickets = createAsyncThunk(
  ...
  async (params: TicketFilterParams, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.detail || 'Failed to load tickets');
    }
  }
);
...
export const fetchTicketById = createAsyncThunk(
  ...
  async (id: number, { rejectWithValue }) => {
    try {
      ...
      const error = err as { response?: { data?: { detail?: string } } };
      return rejectWithValue(error.response?.data?.detail || 'Ticket not found');
    }
  }
);
...
export const assignTicketThunk = createAsyncThunk(
  ...
  async ({ id, data }: { id: number; data: AssignTicketRequest }, { rejectWithValue }) => {
    try {
      ...
      const error = err as { response?: { data?: { detail?: string } } };
      return rejectWithValue(error.response?.data?.detail || 'Failed to assign ticket');
    }
  }
);
...
const ticketsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    const loadingList = (state: TicketsState) => {
      state.isLoading = true;
      state.error = null;
      state.list = [];
      state.total = 0;
    };
    const failedList = (state: TicketsState, action: PayloadAction<unknown>) => {
      state.isLoading = false;
      state.error = action.payload as string;
    };
    const fulfilledList = (
      state: TicketsState,
      action: PayloadAction<PaginatedResponse<TicketBrief>>
    ) => {
      state.isLoading = false;
