...
function EscalationLevelBadge({ level }: { level: number }) {
  ...
  const { label, classes } = cfg[level] ?? {
    label: `L${level}`,
    classes: 'bg-red-100 text-red-800 border border-red-300',
  };
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-bold ${classes}`}
    >
...
export default function EscalatedTicketsPage() {
  ...
  const { tickets, total, page, setPage, isLoading, filters, setFilters, refetch, canSeeAll } =
    useEscalatedTickets();
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading
                ? Array.from({ length: 8 }).map((_, i) => (
                    <SkeletonRow key={i} cols={canSeeAll ? 9 : 8} />
                  ))
                : tickets.map((ticket) => (
                    <tr
