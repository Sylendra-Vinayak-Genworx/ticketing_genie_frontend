...
export default function EscalatedTicketsPage() {
  ...
  const { tickets, total, page, setPage, isLoading, filters, setFilters, refetch, canSeeAll } = useEscalatedTickets()
  ...
  const unassignedCount = tickets.filter(t => !t.assignee_id).length
              ...
              {isLoading
                ...
                : tickets.map(ticket => (
                  <tr key={ticket.ticket_id}
                    ...
                    <td className="px-4 py-3"><SLATimer dueAt={ticket.resolution_due_at} status={ticket.status} isBreached={ticket.is_breached} resolutionSlaCompletedAt={ticket.resolution_sla_completed_at} resolvedAt={ticket.resolved_at} updatedAt={ticket.updated_at} compact /></td>
                    {canSeeAll && (
