...
import {
  PlusCircle, Search, Ticket, RefreshCw, AlertTriangle, UserPlus,
  UserCog, UserCheck, UsersRound, ChevronDown, ChevronUp,
...
function RatePill({ pct, good }: { pct: number; good: boolean }) {
  const color = good
    ...
    : pct >= 30 ? 'bg-red-100 text-red-700'     : pct >= 10 ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'
  const Icon = good
    ? pct >= 70 ? TrendingUp : pct >= 40 ? Minus : TrendingDown
    : pct >= 30 ? TrendingDown : Minus
  return (
...
function TeamPerformanceCard({
  stat, isSelected, onClick,
}: { stat: TeamStat; isSelected: boolean; onClick: () => void }) {
  ...
  const leadName = leadMember?.full_name
    || leadMember?.email
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1.5 mb-3">
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-gray-400 w-16 uppercase tracking-wide shrink-0">Open</span>
          <MiniBar value={stat.open}       max={stat.total} color="bg-blue-400" />
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-gray-400 w-16 uppercase tracking-wide shrink-0">In Prog.</span>
          <MiniBar value={stat.inProgress} max={stat.total} color="bg-indigo-400" />
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-gray-400 w-16 uppercase tracking-wide shrink-0">Resolved</span>
      ...
      <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-100">
        <span className="inline-flex items-center gap-1 text-[11px] text-orange-600 font-medium">
          <Flame className="w-3 h-3" />{stat.escalated} escalated
        </span>
        <span className="inline-flex items-center gap-1 text-[11px] text-red-600 font-medium">
          <ShieldAlert className="w-3 h-3" />{stat.breached} breached
        </span>
...
function PerformanceSummary({ stats }: { stats: TeamStat[] }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
      {[
        { icon: <Ticket       className="w-4 h-4 text-blue-500"   />, label: 'Total Tickets',     value: totalTickets,     border: 'border-blue-300'   },
        { icon: <CheckCircle2 className="w-4 h-4 text-green-500"  />, label: 'Overall Resolution',value: `${overallRes}%`, border: 'border-green-300'  },
        { icon: <ShieldAlert  className="w-4 h-4 text-red-500"    />, label: 'SLA Breached',       value: totalBreached,    border: 'border-red-300'    },
        { icon: <Flame        className="w-4 h-4 text-orange-500" />, label: 'Escalated',          value: totalEscalated,   border: 'border-orange-300' },
        ...
        <>
          <div className="card p-3 flex items-center gap-2 border-l-4 border-green-400 col-span-2">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-gray-400">{best.resolutionRate}% resolution · {best.total} tickets</p>
            </div>
          </div>
          <div className="card p-3 flex items-center gap-2 border-l-4 border-amber-400 col-span-2">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-gray-400">{worst.resolutionRate}% resolution · {worst.breachRate}% breach rate</p>
            </div>
          </div>
        </>
      )}
...
export default function TicketsListPage() {
  ...
  const [filters, setFilters]           = useState<Filters>({
    status: '', severity: '', priority: '', search: '', quickFilter: 'all', teamId: '',
  })
  ...
  const activeTeamName = filters.teamId
    ? teams.find(t => t.id === filters.teamId)?.name
    : null
          ...
          <div className="flex items-center gap-2">
            ...
            {isAdmin && (
              ...
              >
                ...
                Team Performance
                {showPerf ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
              </button>
            )}
            {role === 'user' && (
        ...
        {canSeeAll && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {filters.quickFilter !== 'all' && (
              <button
                onClick={() => { setFilters(f => ({ ...f, quickFilter: 'all' })); setLocalPage(1) }}
