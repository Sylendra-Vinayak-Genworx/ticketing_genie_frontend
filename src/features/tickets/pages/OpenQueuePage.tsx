  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start gap-3 px-4 py-3 rounded-lg bg-sky-50 border border-sky-200 text-sky-800 text-sm">
        ...
        <span>
          Tickets in the <strong>open queue</strong> have no assigned agent. Claim one to take
          ownership and start working on it.
        </span>
      </div>
      ...
      <div className="card overflow-hidden">
                ...
                : tickets.map((ticket) => (
                    ...
                    >
                      <td className="px-4 py-3 font-mono text-xs text-sky-600 font-semibold whitespace-nowrap">
                        ...
                        {ticket.is_breached && (
                          <span className="ml-1.5 text-red-500" title="SLA Breached">
                            ●
                          </span>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            claimTicket(ticket.ticket_id);
                          }}
                          disabled={claimingId === ticket.ticket_id}
        ...
        {!isLoading && tickets.length === 0 && (
          <EmptyState
            icon={<Inbox className="w-12 h-12 text-sky-200" />}
