...
export default function CreateTicketPage() {
  ...
  const { uploadedFiles, handleFileSelect, removeFile, isAnyUploading, hasUploadErrors } =
    useTicketAttachments();

  const { similarTickets, loadingSimilar, solutionFound, handleSolutionFound } =
          ...
          {solutionFound && (
            <div className="bg-green-50 border-l-4 border-green-400 p-4 rounded-r-lg">
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-green-900">Solution Found!</h3>
              ...
              <div className="mt-3 space-y-2">
                {uploadedFiles.map((f, idx) => (
                  ...
                  >
                    ...
                    {f.uploading && <Loader2 className="h-4 w-4 animate-spin text-blue-600 mr-2" />}
                    {!f.uploading && !f.error && (
                      <span className="text-xs text-green-600 mr-2">✓</span>
                    )}
                    {f.error && <span className="text-xs text-red-600 mr-2">✗ Failed</span>}
                    <button
                      type="button"
