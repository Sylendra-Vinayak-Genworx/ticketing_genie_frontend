...
export default function UnassignedTicketsPage() {
  ...
  }, [localPage, filters.severity, filters.priority, fetchAll]);
  useEffect(() => {
    load();
  }, [load]);
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        subtitle={`${total} ticket${total !== 1 ? 's' : ''} awaiting manual assignment`}
        actions={
      ...
      {total > 0 && (
        <div className="flex items-start gap-3 px-4 py-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-sm">
          ...
          <span>
            These tickets have <strong>no assigned agent</strong> — either escalation cleared the
            assignee or auto-assignment failed. Please assign each ticket to an available agent.
          </span>
        </div>
      )}
      ...
      <div className="card overflow-hidden">
                ...
                : filteredList.map((ticket) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs text-gray-500">
                        {(ticket as unknown as { team_id?: string }).team_id ? (
                          <span className="font-medium text-violet-700">
                            {(ticket as unknown as { team_id?: string }).team_id!.slice(0, 8)}…
                          </span>
                        ) : (
        ...
        {!isLoading && filteredList.length === 0 && (
          <EmptyState
            icon={<ClipboardList className="w-12 h-12 text-amber-200" />}
            title="No unassigned tickets"
            description={
              filters.search || filters.severity || filters.priority
                ? 'No tickets match your current filters'
                : 'All tickets have been successfully assigned'
            }
          />
        )}
        {!isLoading && filteredList.length > 0 && (
