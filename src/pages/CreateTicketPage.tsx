...
import type { AreaOfConcern } from '@/features/tickets/services/ticketService'
...
export default function CreateTicketPage() {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    environment: 'PROD',     // fixed — not shown to user
    area_of_concern: '',
    source: 'UI',               // default to UI, hidden from user
  })
  const [errors, setErrors]           = useState<FormErrors>({})
  const [areas, setAreas]             = useState<AreaOfConcern[]>([])
  const [areasLoading, setAreasLoading] = useState(true)

  // Fetch areas of concern on mount
  useEffect(() => {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} noValidate className="card p-6 space-y-5">
        ...
        {/* Area of Concern — dropdown fetched from backend */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">
            Area of Concern
          </label>
          ...
          >
            <option value="">
              {areasLoading ? 'Loading…' : 'Select area of concern…'}
            </option>
            {areas.map(a => (
