...
export default function DashboardPage() {
  ...
  // ── Recent tickets search (overview tab) ─────────────────────────────────
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
      ...
      if (anaDateFrom) params.date_from = new Date(anaDateFrom).toISOString()
      if (anaDateTo)   params.date_to   = new Date(anaDateTo).toISOString()
      if (anaProduct)  params.product   = anaProduct
      const d = await analyticsService.getDashboard(params)
      setAnalytics(d)
      if (d.distribution.by_product.length > 0)
        setProducts(d.distribution.by_product.map(p => p.label))
    } catch {
      setAnaError('Failed to load analytics data')
    } finally {
      setAnaLoading(false)
    }
  }, [isPowerUser, anaDateFrom, anaDateTo, anaProduct])

  useEffect(() => {
    const params = { page: 1, page_size: 10 }
    if (isPowerUser) { fetchAll(params); loadAnalytics() }
    else fetchMy(params)
  }, [role])
  ...
  useEffect(() => {
    if (tab === 'search') loadSearchTickets()
  }, [tab, loadSearchTickets])
  ...
  // Simple counts for non-power users
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={isPowerUser ? (
          <button
            onClick={() => { fetchAll({ page: 1, page_size: 10 }); loadAnalytics() }}
            className="btn-ghost p-2" title="Refresh all">
        ...
        <>
          ...
          {tab === 'overview' && (
            <div className="space-y-5">
                ...
                {showAnaFilter && (
                  <div className="mt-3 flex flex-wrap gap-3 pt-3 border-t border-gray-100">
                    <div>
                      ...
                      <input type="date" value={anaDateFrom} onChange={e => setAnaDateFrom(e.target.value)} className="input-field text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-500 mb-1">To</label>
                      <input type="date" value={anaDateTo} onChange={e => setAnaDateTo(e.target.value)} className="input-field text-sm" />
                    </div>
                    {products.length > 0 && (
                      <div>
                    ...
                    <div className="flex items-end">
                      <button onClick={loadAnalytics} className="btn-primary text-sm py-1.5">Apply</button>
                    </div>
                  </div>
                )}
              </div>
              ...
              {anaLoading ? (
                ...
                <div className="card p-6 text-center text-red-500 text-sm">{anaError}</div>
              ) : summary ? (
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              {analytics && (
                <>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    {/* Priority bar */}
                    <SectionCard title="Open Tickets by Priority" action={<span className="text-xs text-gray-400 bg-green-50 text-green-600 px-2 py-0.5 rounded-full font-medium">Real-time</span>}>
                              ...
                              return (
                                <tr key={a.agent_user_id} className="hover:bg-gray-50/60 transition-colors">
                                  ...
                                  <td className="px-4 py-3">
                                    <span className={a.total_breached > 0 ? 'text-red-600 font-medium' : 'text-gray-300'}>{a.total_breached}</span>
                                  </td>
                                  <td className="px-4 py-3 text-gray-600">
                                    {a.avg_resolution_minutes ? formatMinutes(Math.round(a.avg_resolution_minutes)) : '—'}
                                  </td>
                                  <td className="px-4 py-3">
              ...
              >
                <div className="px-5 py-3 border-b border-gray-50">
                  <div className="relative max-w-sm">
                    ...
                    <input type="text" placeholder="Search recent tickets…" value={recentSearch}
                      onChange={e => setRecentSearch(e.target.value)}
                      className="input-field pl-8 py-1.5 text-sm h-8" />
                  </div>
                </div>
                {isLoading ? (
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {recentTickets.map(ticket => (
                          <tr key={ticket.ticket_id} className="cursor-pointer hover:bg-gray-50/60 transition-colors"
            ...
            <div className="space-y-4">
              <div className="card p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {hasFilters && (
                    <button onClick={() => { setFilters(EMPTY_FILTERS); setSrPage(1) }}
                ...
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search by ticket ID, subject, or keywords…"
                    value={filters.search}
                    onChange={e => { setFilters(f => ({ ...f, search: e.target.value })); setSrPage(1) }}
                    className="input-field pl-9" />
                </div>

                <div className="flex flex-wrap gap-3">
                  <select value={filters.status}
                    onChange={e => { setFilters(f => ({ ...f, status: e.target.value as TicketStatus | '' })); setSrPage(1) }}
                    className="input-field w-auto min-w-[140px]">
                    <option value="">All Statuses</option>
                    {TICKET_STATUSES.map(s => <option key={s} value={s}>{s.replace('_', ' ')}</option>)}
                  </select>
                  <select value={filters.priority}
                    onChange={e => { setFilters(f => ({ ...f, priority: e.target.value as Priority | '' })); setSrPage(1) }}
                    className="input-field w-auto min-w-[120px]">
                    <option value="">All Priorities</option>
                    {PRIORITIES.map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
                  <select value={filters.severity}
                    onChange={e => { setFilters(f => ({ ...f, severity: e.target.value as Severity | '' })); setSrPage(1) }}
                    className="input-field w-auto min-w-[130px]">
                    <option value="">All Severities</option>
                    {SEVERITIES.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                  {products.length > 0 && (
                    <select value={filters.product}
                      onChange={e => { setFilters(f => ({ ...f, product: e.target.value })); setSrPage(1) }}
                      className="input-field w-auto min-w-[130px]">
                      <option value="">All Products</option>
                      {products.map(p => <option key={p} value={p}>{p}</option>)}
                    </select>
                  ...
                  <input type="date" value={filters.date_from} title="From date"
                    onChange={e => { setFilters(f => ({ ...f, date_from: e.target.value })); setSrPage(1) }}
                    className="input-field text-sm" />
                  <input type="date" value={filters.date_to} title="To date"
                    onChange={e => { setFilters(f => ({ ...f, date_to: e.target.value })); setSrPage(1) }}
                    className="input-field text-sm" />
                </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    {Object.entries(filters).filter(([, v]) => !!v).map(([key, val]) => (
                      <span key={key} className="inline-flex items-center gap-1 px-2.5 py-1 bg-violet-50 text-violet-700 text-xs font-medium rounded-full border border-violet-200">
                        ...
                        <button onClick={() => { setFilters(f => ({ ...f, [key]: '' })); setSrPage(1) }}>
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    ))}
              ...
              <SectionCard
                title={srLoading ? 'Loading…' : `${srTotal} ticket${srTotal !== 1 ? 's' : ''} found`}
                noPad
              >
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                      ...
                      {srLoading
                        ? Array.from({ length: 8 }).map((_, i) => (
                          <tr key={i}>{Array.from({ length: 8 }).map((__, j) => (
                            <td key={j} className="px-4 py-3"><div className="h-3 bg-gray-100 rounded animate-pulse w-20" /></td>
                          ))}</tr>
                        ))
                        : srTickets.length === 0
          ...
          {tab === 'reports' && (
            <div className="space-y-5">
              ...
              {anaError   && <div className="card p-8 text-center text-red-500">{anaError}</div>}
              {analytics && !anaLoading && (
                <>
                          ...
                          <div key={p.name} className="flex items-center gap-3">
                            ...
                            <div className="flex-1 h-5 bg-gray-100 rounded-full overflow-hidden">
                              <div className="h-full rounded-full transition-all"
                                style={{
                                  width: priorityData.length && Math.max(...priorityData.map(x => x.tickets)) > 0
                                    ? `${p.tickets / Math.max(...priorityData.map(x => x.tickets)) * 100}%` : '0%',
                  ...
                  {/* Product report */}
                  <SectionCard title="Tickets by Product">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        {productData.map((p, i) => (
                          <div key={p.name} className="flex items-center gap-3">
                            <span className="text-xs text-gray-500 w-24 truncate">{p.name}</span>
                            <div className="flex-1 h-4 bg-gray-100 rounded-full overflow-hidden">
                              <div className="h-full rounded-full transition-all"
                                style={{
                                  width: productData.length && Math.max(...productData.map(x => x.tickets)) > 0
                                    ? `${p.tickets / Math.max(...productData.map(x => x.tickets)) * 100}%` : '0%',
                                  backgroundColor: PIE_COLORS[i % PIE_COLORS.length],
                                }} />
                            </div>
                            <span className="text-sm font-semibold text-gray-900 w-6 text-right">{p.tickets}</span>
                          </div>
                        ))}
                      </div>
                      <ResponsiveContainer width="100%" height={180}>
                        <PieChart>
                          <Pie data={productData.map(p => ({ name: p.name, value: p.tickets }))}
                                ...
                                <tr key={a.agent_user_id} className="hover:bg-gray-50/60 transition-colors">
                                  ...
                                  <td className="px-4 py-3">
                                    {a.avg_resolution_minutes
                                      ? <span className="font-medium text-violet-700">{formatMinutes(Math.round(a.avg_resolution_minutes))}</span>
        ...
        <div className="space-y-6">
          ...
          >
            {isLoading ? (
              ...
              <div className="py-12 text-center text-gray-500 text-sm">
                <Ticket className="w-10 h-10 text-gray-200 mx-auto mb-3" />No tickets yet
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
