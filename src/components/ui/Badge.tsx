...
import { STATUS_CONFIG, PRIORITY_CONFIG, SEVERITY_CONFIG } from '@/config/constants';
...
export function StatusBadge({ status, className }: StatusBadgeProps) {
  ...
  return <span className={cn('badge', cfg.bg, cfg.color, className)}>{cfg.label}</span>;
}
...
export function Badge({ children, variant = 'default', className }: BadgeProps) {
  return <span className={cn('badge', VARIANT_STYLES[variant], className)}>{children}</span>;
}
